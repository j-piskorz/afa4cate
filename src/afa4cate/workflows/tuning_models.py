...
def tuning(config: DictConfig):
    ...
    if config.cate_model.model_name == "tarnet":
        job_dir = Path(__file__).parent / "tuning_files" / config.cate_model.model_name
    else:
    ...
    if config.cate_model.model_name == "deep_kernel_gp":
        space["kernel"] = tune.choice(["RBF", "Matern12", "Matern32", "Matern52"])
        space["num_inducing_points"] = tune.choice([100, 200, 500])

    # Create the training dataset and put it in the Ray object store
    ...
    def func(params):
        ...
        _ = train_or_load_model(config.cate_model, ds_train, ds_valid, None, device="cuda:0")
